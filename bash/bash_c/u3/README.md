...
while true; do
  ...
  # =====================================================================
  # FITUR: BUAT FILE BARU DI FOLDER / SUB-FOLDER AKTIF ([n]) - VERSI INSTAN
  # =====================================================================
  # =====================================================================
  # FITUR: BUAT FILE BARU DI FOLDER / SUB-FOLDER AKTIF ([n]) - PERBAIKAN TOTAL
  # =====================================================================
  if [[ "$pilihan" == "n" || "$pilihan" == "N" ]]; then
    ...
    # 2. Tarik daftar sub-folder dari REMOTE (GitHub)
    mapfile -t remote_dirs < <(git -C "$rp" ls-tree -r -d --name-only "origin/${current_branch}" 2>/dev/null)

    # 3. Tarik daftar sub-folder dari LOKAL (Termux) - PERBAIKAN PARAMETER: -type d
    local_dirs=()
    if [ -d "$rp" ]; then
    ...
    # 5. Tampilkan daftar semua folder & sub-folder secara instan
    clear
    echo "========================================"
    ...
    if [[ "$folder_num_pilihan" =~ ^[0-9]+$ ]] && [ "$folder_num_pilihan" -ge 1 ] && [ "$folder_num_pilihan" -le "${#sorted_dirs[@]}" ]; then
      ...
      # Gabungkan jalur lengkap lokal
      full_new_file_path="${rp}/${target_folder_path}/${nama_file_murni}"
