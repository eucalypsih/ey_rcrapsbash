...
aa: 
Berikut adalah modifikasi kode pada bagian **Menu Interaktif Seleksi Folder** yang menyertakan otomatisasi `git rm` agar berkas yang dihapus langsung tercatat di *staging area* Git, beserta modifikasi menu tampilannya.

## 1. Modifikasi Tampilan Menu Utama
Tambahkan opsi `[d]` pada baris menu tampilan visual:
```bash
  echo -e "${CYAN}----------------------------------------${NC}"
...
## 2. Modifikasi Struktur Blok `case "$pilihan" in`
Sisipkan blok logika `[dD]`) berikut ke dalam struktur `case` Anda:
```bash
    [dD])
      # =====================================================================
      ...
      if [ ${#active_folders[@]} -eq 0 ]; then
        ...
        echo -e "\n${YELLOW}[+] Memetakan seluruh file dan folder lokal...${NC}"
        ...
        if [ ${#all_local_items[@]} -eq 0 ]; then
          echo -e "\n${YELLOW}[!] Tidak ada file atau sub-folder lokal yang dapat dihapus.${NC}"
          echo -e -n "${YELLOW}Tekan [Enter] untuk kembali...${NC}"
          read -r
        else
          clear
          echo -e "${CYAN}========================================${NC}"
          echo -e "${CYAN}     HAPUS FILE / SUB-DIREKTORI         ${NC}"
          echo -e "${CYAN}========================================${NC}"
          ...
          echo -e "${CYAN}----------------------------------------${NC}"
          echo -e -n "${YELLOW}[~] Pilih nomor target yang ingin dihapus dari Git & Lokal: ${NC}"
          ...
          if [[ "$hapus_idx_num" =~ ^[0-9]+$ ]] && [ "$hapus_idx_num" -ge 1 ] && [ "$hapus_idx_num" -le "${#all_local_items[@]}" ]; then
            ...
            echo -e -n "${RED}[⚠️] Apakah Anda yakin ingin menghapus '${target_hapus}' dari Git & Penyimpanan? (y/n): ${NC}"
            read -r konfirmasi_item_hapus

            if [[ "$konfirmasi_item_hapus" =~ ^[yY]$ ]]; then
              echo -e "${CYAN}[~] Menghapus target menggunakan git rm...${NC}"
              ...
              if git -C "$rp" ls-files --error-unmatch "$target_hapus" &>/dev/null; then
                ...
                echo -e "${GREEN}[✓] Sukses: '${target_hapus}' dihapus dan dicatat di staging area Git.${NC}"
              else
                # Jika file baru (untracked) dan belum di-commit, hapus secara fisik langsung
                rm -rf "$full_target_path"
                echo -e "${GREEN}[✓] Sukses: Berkas untracked '${target_hapus}' telah dihapus fisik.${NC}"
              fi
              sleep 1.5
            else
...
1. Otomatis Masuk Staging (`git rm -rf`): Jika berkas atau folder tersebut sudah pernah ada di repositori GitHub (tracked), skrip akan mengeksekusi penghapusan lewat Git. Saat Anda menekan opsi `[q]` untuk keluar, perubahan penghapusan ini akan langsung ditawarkan untuk otomatis di-commit dan di-*push*.
2. Penanganan Berkas Baru (*Untracked*): Jika Anda memilih file baru yang belum pernah di-commit ke Git, sistem secara cerdas akan langsung menghapusnya secara fisik (`rm -rf`) tanpa menimbulkan eror pada perintah Git.
...
while true; do
  ...
  echo -e "${CYAN}========================================${NC}"
  echo -e -n "${YELLOW}[~] Masukkan pilihan Anda: ${NC}"
  read -r repo_pilihan

  # OPSI KELUAR
  if [[ "$repo_pilihan" =~ ^[qQ]$ ]]; then
    echo -e "\n${RED}[X] Proses dibatalkan. Keluar dari skrip.${NC}"
    exit 0
  fi # end if q
  ...
  if [[ "$repo_pilihan" =~ ^[hH]$ ]]; then
    ...
    echo -e "${RED}----------------------------------------${NC}"
    echo -e -n "${YELLOW}[~] Pilih nomor repo yang ingin dibuang dari daftar: ${NC}"
    read -r hapus_num
    ...
    if [[ "$hapus_num" =~ ^[0-9]+$ ]] && [ "$hapus_num" -ge 1 ] && [ "$hapus_num" -le "${#valid_repos[@]}" ]; then
      ...
      if [[ "$konfirmasi_hapus" =~ ^[yY]$ ]]; then
        ...
        if [ -d "$target_folder_fisik" ]; then
          ...
          echo -e -n "${RED}[⚠️] HAPUS PERMANEN folder fisik tersebut beserta seluruh representsi filenya? (y/n): ${NC}"
          read -r konfirmasi_fisik

          if [[ "$konfirmasi_fisik" =~ ^[yY]$ ]]; then
            echo -e "${CYAN}[~] Memusnahkan folder fisik lokal: ${repo_terhapus}...${NC}"
...
if [ -d "$rp/.git" ]; then
  ...
  if [ -n "$detected_branch" ]; then
    ...
    echo -e "${RED}[X] Error: Branch tidak ditemukan!${NC}"
    exit 1
...
if [ -e "$rp" ] && [ -f "$rp" ] && [ ! -d "$rp" ]; then
  ...
  if [[ "$hapus_file" =~ ^[yY]$ ]]; then
    echo -e "${CYAN}[~] Menghapus file penghalang...${NC}"
    rm -f "$rp"
  else
...
if [ ! -d "$rp" ]; then
  echo -e "${CYAN}[~] Mengkloning repositori ke ${rp}...${NC}"
  ...
  echo -e "${GREEN}[✓] Kloning berhasil dilakukan.${NC}"
  sleep 0.5
...
if [ -z "$current_sparse" ]; then
  ...
  echo -e "${CYAN}[~] Memperbarui informasi struktur folder dari remote...${NC}"
  ...
  if git -C "$rp" fetch -q origin "$current_branch" 2>/dev/null; then
    ...
    echo -e -n "${YELLOW}[~] Apakah ingin memuat menu dengan cache lokal lama? (y/n): ${NC}"
    read -r konfirmasi
...
while true; do
  clear
  echo -e "${CYAN}========================================${NC}"
  echo -e "${CYAN}   SISTEM SELEKSI SPARSE-CHECKOUT       ${NC}"
  echo -e "${CYAN}========================================${NC}"
  echo -e "${YELLOW}Daftar folder otomatis dari Remote:${NC}"
  ...
  case "$pilihan" in
    [qQ])
      ...
      if [ -n "$perubahan_lokal" ]; then
        echo -e "\n${RED}[⚠️] PERINGATAN: Ada file yang telah Anda ubah/tambahkan secara lokal!${NC}"
        ...
        echo -e "${CYAN}----------------------------------------${NC}"
        echo -e -n "${YELLOW}[~] Pilihan Anda (1/2/3): ${NC}"
        read -r aksi_keluar
        ...
        if [ "$aksi_keluar" == "1" ]; then
          ...
          echo -e -n "${YELLOW}[~] Tekan [Enter] untuk auto-msg, atau ketik pesan manual: ${NC}"
          read -r pesan_commit
          ...
          echo -e "${RED}[⚠️] Membuang perubahan lokal dan melakukan paksa checkout...${NC}"
      ...
      echo -e "\n${CYAN}========================================${NC}"
      echo -e "${CYAN}       PERBANDINGAN STATUS COMMIT       ${NC}"
      echo -e "${CYAN}========================================${NC}"
      ...
      if [ "${ahead_commits:-0}" -gt 0 ]; then
        ...
        echo -e -n "${YELLOW}[~] Apakah Anda yakin ingin melakukan PUSH ke GitHub sekarang? (y/n): ${NC}"
        read -r konfirmasi_push
      
        if [[ "$konfirmasi_push" =~ ^[yY]$ ]]; then
          echo -e "${CYAN}[~] Melakukan git push origin ${current_branch}...${NC}"
    ...
    [dD])
      ...
      if [ ${#active_folders[@]} -eq 0 ]; then
        ...
        if [ ${#all_local_items[@]} -eq 0 ]; then
          ...
          echo -e "${CYAN}----------------------------------------${NC}"
          echo -e -n "${YELLOW}[~] Pilih nomor target yang ingin dihapus: ${NC}"
          read -r hapus_idx_num
    ...
    [nN])
      ...
      if ! command -v micro &> /dev/null; then
        ...
        if [ ${#active_folders[@]} -eq 0 ]; then
          ...
          echo -e "\n${CYAN}[~] Memetakan seluruh struktur sub-folder secara mendalam...${NC}"
          ...
          if [ ${#sorted_dirs[@]} -eq 0 ]; then
            ...
            clear
            echo -e "${CYAN}========================================${NC}"
            echo -e "${CYAN}   PILIH STRUKTUR FOLDER TUJUAN         ${NC}"
            echo -e "${CYAN}========================================${NC}"
            for i in "${!sorted_dirs[@]}"; do
              printf " [%d] /%s/\n" $((i+1)) "${sorted_dirs[$i]}"
            done # end for display dirs
            echo -e "${CYAN}----------------------------------------${NC}"
            echo -e -n "${YELLOW}[~] Pilih nomor folder tempat menaruh file baru: ${NC}"
            ...
            if [[ "$folder_num_pilihan" =~ ^[0-9]+$ ]] && [ "$folder_num_pilihan" -ge 1 ] && [ "$folder_num_pilihan" -le "${#sorted_dirs[@]}" ]; then
              ...
              echo -e -n "${YELLOW}[~] Masukkan NAMA FILE BARU (Contoh: ${GREEN}script.py${YELLOW}): ${NC}"
              ...
              if [ -z "$nama_file_murni" ]; then
                ...
                echo -e "\n${CYAN}[~] Membuat dan membuka file baru dengan micro...${NC}"
                sleep 1.5
      ...
      if ! command -v micro &> /dev/null; then
        ...
        if [ ${#active_folders[@]} -eq 0 ]; then
          ...
          if [ ${#valid_files[@]} -eq 0 ]; then
            ...
            clear
            echo -e "${CYAN}========================================${NC}"
            echo -e "${CYAN}   DAFTAR FILE YANG DAPAT DIEDIT        ${NC}"
            echo -e "${CYAN}========================================${NC}"
            for i in "${!valid_files[@]}"; do
            printf " [%d] %s\n" $((i+1)) "${valid_files[$i]}"
            done # end for display files
            echo -e "${CYAN}----------------------------------------${NC}"
            echo -e -n "${YELLOW}[~] Pilih nomor file yang ingin diedit dengan micro: ${NC}"
            read -r num_pilihan
            ...
            if [[ "$num_pilihan" =~ ^[0-9]+$ ]] && [ "$num_pilihan" -ge 1 ] && [ "$num_pilihan" -le "${#valid_files[@]}" ]; then
              ...
              echo -e "\n${CYAN}[~] Membuka ${selected_file} dengan micro...${NC}"
              sleep 0.5
    ...
    [0-9]*)
      ...
      if [[ "$pilihan" =~ ^[0-9]+$ ]] && [ "$pilihan" -ge 1 ] && [ "$pilihan" -le "${#targets[@]}" ]; then
        ...
        if echo "$current_sparse" | grep -qE "^/?${selected_folder}/?$"; then
          echo -e "\n${RED}[!] Ops! Folder /${selected_folder} sudah aktif ada di dalam daftar sparse-list.${NC}"
        else
          echo -e "\n${CYAN}[~] Menambahkan /${selected_folder} ke sparse-checkout...${NC}"
      ...
      echo ""
      echo -e -n "${YELLOW}Tekan [Enter] untuk melanjutkan...${NC}"
      read -r
      ;;
