...
aa: 
Berikut adalah modifikasi kode pada bagian **Menu Interaktif Seleksi Folder** (di dalam blok `while true` kedua). Opsi baru berupa **[d] Hapus File atau Sub-Folder** telah ditambahkan ke dalam menu dan struktur `case "$pilihan" in`.

## 1. Modifikasi Tampilan Menu Menu Utama
...
```bash
  ...
  echo -e " [d] ${RED}Hapus File atau Sub-Folder${NC}" # <-- TAMBAHKAN BARIS INI
...
Sisipkan blok logika `[dD]`) berikut di atas atau di bawah blok `[nN]`) / `[eE]`):
```base
    [dD])
      # =====================================================================
      # FITUR BARU: HAPUS FILE ATAU SUB-DIREKTORI
      # =====================================================================
      active_folders=()
      ...
      if [ ${#active_folders[@]} -eq 0 ]; then
        ...
        if [ ${#all_local_items[@]} -eq 0 ]; then
          ...
          if [[ "$hapus_idx_num" =~ ^[0-9]+$ ]] && [ "$hapus_idx_num" -ge 1 ] && [ "$hapus_idx_num" -le "${#all_local_items[@]}" ]; then
            ...
            echo -e -n "${RED}[⚠️] Apakah Anda yakin ingin menghapus '${target_hapus}' secara permanen? (y/n): ${NC}"
            read -r konfirmasi_item_hapus

            if [[ "$konfirmasi_item_hapus" =~ ^[yY]$ ]]; then
              # Eksekusi penghapusan fisik
              rm -rf "$full_target_path"
              echo -e "${GREEN}[✓] Sukses: '${target_hapus}' telah dihapus dari penyimpanan lokal.${NC}"
              sleep 1.5
            else
...
1. Dinamis & Terarah: Sistem hanya akan membaca dan menampilkan representasi file/sub-folder lokal yang berada di bawah direktori tingkat pertama (induk) yang statusnya **sudah aktif**.
2. Fleksibel: Menggunakan perintah `find` yang secara cerdas mendeteksi tipe komponen. Jika item berupa direktori, terminal memberi visual tag `[ Folder ]`. Jika berupa berkas biasa, akan diberi label `[ File ]`.
3. Aman Permanen: Menggunakan `rm -rf` setelah melalui konfirmasi pengguna (`y/n`) agar direktori kosong maupun sub-direktori dengan struktur file di dalamnya dapat dibersihkan secara tuntas.
