...
memodifikasi skrip agar membaca file eksternal `repo.txt` dan menambahkan sebuah **Menu Seleksi Repositori Utama** sebelum masuk ke menu *sparse-checkout* folder.
...
if [ ! -f "$repo_file" ]; then
    echo "Membuat file konfigurasi '${repo_file}' default..."
    echo -e "ey_rcrapsbash\ney_repo2\ney_repo3\ney_repo4" > "$repo_file"
fi

# Membaca daftar repo dari file ke dalam array secara dinamis
...
if [ ${#valid_repos[@]} -eq 0 ]; then
    echo "Error: File '${repo_file}' kosong! Silakan isi nama repositori terlebih dahulu."
    exit 1
fi

# =====================================================================
# MENU SELEKSI REPOSITORI UTAMA
# =====================================================================
while true; do
    clear
    echo "========================================"
    echo "       PILIH REPOSITORI UTAMA           "
    echo "========================================"
    ...
    echo "========================================"
    echo -n "Pilih nomor repositori yang ingin dibuka: "
    read -r repo_pilihan
    ...
    if [[ "$repo_pilihan" =~ ^[0-9]+$ ]] && [ "$repo_pilihan" -ge 1 ] && [ "$repo_pilihan" -le "${#valid_repos[@]}" ]; then
        ...
        echo "Pilihan tidak valid! Silakan pilih nomor yang tertera."
        sleep 1
    fi
done
...
if [ ! -f "$repo_file" ]; then
  ...
  echo -e "${YELLOW}[+] Membuat file '${repo_file}' default untuk Anda...${NC}"
  echo -e "ey_rcrapsbash\ney_repo2\ney_repo3\ney_repo4" > "$repo_file"
fi
...
# TAMPILKAN MENU SELEKSI REPOSITORI UTAMA
while true; do
  clear
  echo "========================================"
  echo "       PILIH REPOSITORI UTAMA           "
  echo "========================================"
  for i in "${!valid_repos[@]}"; do
  printf " [%d] %s\n" $((i+1)) "${valid_files_or_dirs:-${valid_repos[$i]}}"
  done
  echo "----------------------------------------"
  echo " [q] Keluar dari Skrip"
  echo "========================================"
  ...
  if [[ "$repo_pilihan" =~ ^[0-9]+$ ]] && [ "$repo_pilihan" -ge 1 ] && [ "$repo_pilihan" -le "${#valid_repos[@]}" ]; then
    ...
    echo -e "${RED}[X] Pilihan tidak valid!${NC}"
    sleep 1
  fi
done
